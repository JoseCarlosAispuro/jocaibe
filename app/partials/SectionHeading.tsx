...
function HeadingLine({ text, muted, delay }: { text: string; muted?: boolean; delay: number }) {
  const outerRef = useRef<HTMLSpanElement>(null)
  const inView = useInView(outerRef, { once: true, margin: '0px 0px -5% 0px' })

  return (
    <span ref={outerRef} style={{ display: 'block', overflow: 'hidden', paddingBottom: '0.08em' }}>
      <motion.span
        ...
        style={{ display: 'block', color: muted ? 'var(--fg-2)' : undefined }}
      >
        {text}
      </motion.span>
    </span>
...
export default function SectionHeading({ lines, right }: SectionHeadingProps) {
  return (
    <div className="grid grid-cols-12 gap-8 items-baseline">
      <h2 className="col-span-10 [font-family:var(--font-display)] [font-size:var(--fs-h1)] font-medium tracking-[-0.03em] leading-none text-(--fg-0)">
        {lines.map((ln, i) => (
